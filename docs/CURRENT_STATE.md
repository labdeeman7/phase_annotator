...
- Draggable boundary editing is not implemented yet. C3.1-C3.5 and C4.1 bounded in-memory undo/redo are manually accepted; history is not persisted across application or video loads.
...
Implement C4.2 boundary handles and hit testing. Improving status-bar errors into prominent top-of-window notifications remains deferred to the beautification backlog.
