...
- Boundary dragging is not implemented yet. C3.1-C3.5 and C4.1-C4.2 are manually accepted. History is not persisted across application or video loads.
...
Implement C4.3 transient drag preview and one-command commit. Improving status-bar errors into prominent top-of-window notifications remains deferred to the beautification backlog.
